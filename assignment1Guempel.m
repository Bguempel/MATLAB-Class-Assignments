...
%
% Answer: clears all variables in Workspace
%
...
%
% Answer: [h,p] = ttest() This test allows you to test for the null
% hypothesis in a set of data at a 5% significance level. The simpler version is a one-sample t-test
% using data from one source (ex: x). "p" will return the actual p-value of
% the test validating if the data is within the .05 threshold or not.
%
...
%
% Answer: Condition controlled loops and count controlled loops. Condition
% - repeats the statements as long as the conditions are true (no finite
% number of loops). Count - repeats statements a fixed number of times (the
% resulting loop has a definite ending)
% 
...
originalWeightOz = 100*35.274;

who originalWeightLbs
whos originalWeightLbs
who originalWeightOz
whos originalWeightOz
...
originalVecSub = originalVec - 3;

% - count how many are positive

orignialVecPosNum = sum(originalVec >0);
...
% code so that another user can understand what your code does. 
 ...
 tBottomCm = 12;
 tHeightCm = 5;
trapArea = (tTopCm + tBottomCm) / 2 * (tHeightCm);
...
promptTotalRev = "What is the total revenue?";
promptTotalCosts = "What is the total costs?";
totalRev = input(promptTotalRev);
...
promptCopycat = "Type what you want to say to the world";
txt = input(promptCopycat, "s");
disp(txt);
